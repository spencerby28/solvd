...
<aside class="bg-gray-100 border-x border-gray-200 shadow-lg relative rounded-l-3xl flex flex-col h-screen overflow-hidden transition-all duration-300 {isCollapsed ? 'w-16' : 'w-80'} group">	
    <div class="p-4 relative">
        <div class="flex items-start gap-4">
            ...
            {#if !isCollapsed}
                <div class="min-w-0 flex-1" transition:slide|local>
                    {#if loading}
                        <div class="space-y-2">
        ...
        {#if !isCollapsed}
            <div class="mt-4 pt-4 border-t border-gray-200 grid grid-cols-[auto_1fr] gap-x-6 gap-y-3" transition:slide|local>
                {#if loading}
    ...
    <div class="p-4 border-t border-gray-200">
        {#if !isCollapsed}
            {#if selectedAssignees.size > 0}
                <div class="flex items-center justify-between mb-4" transition:slide|local>
    ...
    <div class="p-4 border-t border-gray-200">
        ...
        {/if}
        <div class="grid {isCollapsed ? 'grid-cols-1 gap-2' : 'grid-cols-2 gap-2'}">
            <RippleButton 
                class="p-2 text-sm bg-white hover:bg-gray-50 border border-gray-200 rounded-lg flex items-center {isCollapsed ? 'justify-center' : ''}"
                rippleColor="#16a34a"
                duration="500ms"
            >
    ...
    <!-- Ticket Summary and Activity Timeline - Only show when not collapsed -->
    {#if !isCollapsed}
        <!-- Ticket Summary -->
        <div class="p-4 border-t border-gray-200" transition:slide|local>
        ...
        <!-- Activity Timeline -->
        <div class="px-4 border-t border-gray-200 flex-1 flex flex-col min-h-0" transition:slide|local>
